...
#include "Platforms/SYCL/SYCLallocator.hpp"
#include "Platforms/SYCL/syclBLAS.hpp"
#include "Platforms/SYCL/syclSolver.hpp"
#include "Platforms/SYCL/sycl_determinant_helper.hpp"

#define USE_SYCL_EVENT
...
public:
                                                                     ...
                                                                     sycl::queue& m_queue)
  {
    ...
    auto c_event_1 = syclSolver::getri(m_queue, norb, Ainv_gpu.data(), norb, ipiv.data(), workspace.data(), getri_ws);

    m_queue.memcpy(Ainv.data(), Ainv_gpu.data(), Ainv.size() * sizeof(TMAT), {c_event_1}).wait();
#else
    ...
    syclSolver::getri(m_queue, norb, Ainv_gpu.data(), norb, ipiv.data(), workspace.data(), getri_ws).wait();
                                                                      ...
                                                                      sycl::queue& m_queue)
  {
    ...
    auto c_event_1 = syclSolver::getri(m_queue, norb, Mat1_gpu.data(), norb, ipiv.data(), workspace.data(), getri_ws);

    auto t_event_1 = syclBLAS::copy_n(m_queue, Mat1_gpu.data(), Mat1_gpu.size(), Ainv_gpu.data(), {c_event_1});

    m_queue.memcpy(Ainv.data(), Ainv_gpu.data(), Ainv.size() * sizeof(TMAT), {t_event_1}).wait();
#else
    //use Ainv_gpu for transpose
    m_queue.memcpy(Ainv_gpu.data(), logdetT.data(), logdetT.size() * sizeof(TMAT)).wait();
    //transpose
    syclBLAS::transpose(m_queue, Ainv_gpu.data(), norb, Ainv_gpu.cols(), Mat1_gpu.data(), norb, Mat1_gpu.cols()).wait();
    ...
    syclBLAS::copy_n(m_queue, Mat1_gpu.data(), Mat1_gpu.size(), Ainv_gpu.data()).wait();
